                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-justified justify-content-center">
                                    ...
                                    @if(!$translation->is_default)
                                    @if($translation->is_active)
                                    <a href="{{ route('admin.disableTranslation', $translation->id) }}"
                                        class="btn btn-sm btn-primary ml-1" data-popup="tooltip"
                                        title="Disable Translation" data-placement="bottom"> <i
                                            class="icon-switch2"></i> </a>
                                    @else
                                    <a href="{{ route('admin.disableTranslation', $translation->id) }}"
                                        class="btn btn-sm btn-danger ml-1" data-popup="tooltip"
                                        title="Enable Translation" data-placement="bottom"> <i class="icon-switch2"></i>
                                    </a>
                                    @endif
                                    @endif

                                    <a href="@if($translation->is_default) javascript:void(0) @else {{ route('admin.makeDefaultLanguage', $translation->id) }} @endif"
                                        class="btn btn-sm btn-dark ml-1 @if($translation->is_default) badge-success btn-success cursor-disabled @endif"
                                        @if(!$translation->is_default) data-popup="tooltip" title="Make
                                        {{ $translation->language_name }} as Default Language" data-placement="bottom"
                                        @endif> <i class="icon-checkmark4"></i> </a>
                                </div>
                            </td>
                        </tr>
                        @endforeach
